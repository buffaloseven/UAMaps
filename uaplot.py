...
def main():
    ...
    parser = OptionParser(conflict_handler="resolve",
                          usage=usage, version="%prog 1.0 By Kyle Ziolkowski")
    parser.add_option("-h", "--help", dest="help",
                      help="--am or --pm for 12z or 00z obs")
    parser.add_option("--latest", "--latest", dest="latest",
                      action="store_true", help="Get 00z obs", default=False)
    parser.add_option("--date", "--date", dest="date", type="str",
                      help="date in format YYYYMMDDHH. Note: HH must be 12 or 00 for 12z or 00z maps", default=False)
    parser.add_option("--td", "--td", dest='td', action="store_true",
                      help="Plot dewpoint instead of dewpoint depression", default=False)
    parser.add_option("--te", "--te", dest='te', action="store_true",
                      help="Plot Theta-e instead of temperatures for 925/850/700 mb", default=False)
    parser.add_option("--levels", "--levels", dest='levels', type="str",
                      help="Levels in which to plot. Use levels=850,500 to plot specific levels. This option is not required to plot all levels (250, 300, 500, 700, 850, 925).", default='All')
    parser.add_option("--compress-output", "--compress-output", dest="compress", action="store_true",
                      help="Compress the output PNGs when saving; will add extra to the runtime.", default=False)
    ...
    if opt.latest == False and opt.date == False:
        parser.error(
            'No option for plotting selected. Please select --latest for latest UAMaps or --date=YYYYMMDDHH for a specific time.')
    td_option = opt.td
    te_option = opt.te
    if opt.date != False:
    ...
    ds = xr.open_dataset(
        'https://thredds.ucar.edu/thredds/dodsC/grib/NCEP/GFS/Global_0p5deg_ana/GFS_Global_0p5deg_ana_{0:%Y%m%d}_{0:%H}00.grib2'.format(date)).metpy.parse_cf()
    ...
    if (opt.combine_pdf is True):
        ...
        images[0].save(
            pdf, "PDF", save_all=True, append_images=images[1:]
        )
...
def getData(station_file, date, hh):
    ...
    for station, lat, lon in zip(stations, lats, lons):
        ...
        try:
            ...
            print('...failed.')
            pass
    print('Data retrieved. Fetched ' + str(data_success) + '/' + str(total_stations) +
          "(" + str(round(100*(data_success/total_stations))) + "%) of requested stations.")
...
def dataDict(data_arr):
    ...
    data['eastward_wind'], data['northward_wind'] = data_arr[2] * \
        units('knots'), data_arr[3] * units('knots')
    data['height'] = data_arr[6] * units('meters')
    data['pressure'] = data_arr[7] * units('hPa')
    data['thetae'] = te(data['pressure'], data['air_temperature'],
...
def mapbackground():
    ...
    coastlines = feat.NaturalEarthFeature(
        'physical', 'coastline', '50m', facecolor='none')
    lakes = feat.NaturalEarthFeature(
        'physical', 'lakes', '50m', facecolor='none')
    countries = feat.NaturalEarthFeature(
...
def uaPlot(data, level, date, save_dir, ds, hour, td_option, te_option, date_option, image_compress, png_colours, thumbnails, thumbnail_size, long_filenames, smaller_images):
    ...
    custom_layout.add_value('NW', 'air_temperature',
                            fmt='.0f', units='degC', color='darkred')

    # decide on the height format based on the level
    if level == 250 or level == 300:
        custom_layout.add_value('NE', 'height', fmt=lambda v: format(v, '1')[
                                1:4], units='m', color='black')
        cint = 120
        tint = 5
        ...
        tmpk = ds.Temperature_isobaric.metpy.sel(
            vertical=level*100, lat=slice(85, 15), lon=slice(360-200, 360-10))*units.degK
        smooth_tmpc = (mpcalc.smooth_n_point(
            tmpk.data, 9, 10)).to('degC').squeeze()
        ...
        level_idx = list(pres.m).index(
            ((level * units('hPa')).to(pres.units)).m)

    if level == 500:
        custom_layout.add_value('NE', 'height', fmt=lambda v: format(v, '1')[
                                0:3], units='m', color='black')
        cint = 60
        tint = 5
        lats = ds.lat.sel(lat=slice(85, 15)).values
        lons = ds.lon.sel(lon=slice(360-200, 360-10)).values
        hght = ds.Geopotential_height_isobaric.metpy.sel(
            vertical=level * 100,  lat=slice(85, 15), lon=slice(360-200, 360-10))*units.hPa
        smooth_hght = mpcalc.smooth_n_point(hght, 9, 10).squeeze()
        tmpk = ds.Temperature_isobaric.metpy.sel(
            vertical=level*100, lat=slice(85, 15), lon=slice(360-200, 360-10))*units.degK
        smooth_tmpc = (mpcalc.smooth_n_point(
            tmpk.data, 9, 10)).to('degC').squeeze()
        abs_vort = ds.Absolute_vorticity_isobaric.metpy.sel(
            vertical=level*100, lat=slice(85, 15), lon=slice(360-200, 360-10)).squeeze()
        avort = ndimage.gaussian_filter(
            abs_vort, sigma=3, order=0) * units('1/s')
    ...
    if level == 850:
        ...
        if td_option == True:
            custom_layout.add_value(
                'SW', 'dew_point_temperature', units='degC', color='green')
            temps = 'Td'
        if td_option == False:
            custom_layout.add_value('SW', 'tdd', units='degC', color='green')
            temps = 'Tdd'
    ...
    hght = ds.Geopotential_height_isobaric.metpy.sel(
        vertical=level * 100,  lat=slice(85, 15), lon=slice(360-200, 360-10))*units.hPa
    smooth_hght = mpcalc.smooth_n_point(hght, 9, 10).squeeze()
    cs = ax.contour(lons, lats, smooth_hght.values, range(
        0, 20000, cint), colors='black', transform=ccrs.PlateCarree())
    clabels = plt.clabel(cs, fmt='%d', colors='white',
                         inline_spacing=5, use_clabeltext=True, fontsize=14)
    ...
    if level == 250 or level == 300:
        ...
        cs2 = ax.contour(hght.lon, hght.lat, smooth_tmpc.m, range(-60,
                                                                  51, tint), colors='red', transform=ccrs.PlateCarree())
        clabels = plt.clabel(cs2, fmt='%d', colors='red',
                             inline_spacing=5, use_clabeltext=True, fontsize=22)
    ...
    if level == 500:
        # Plot Dashed Contours of Temperature
        cs2 = ax.contour(lons, lats, smooth_tmpc.m, range(-60,
                                                          51, tint), colors='red', transform=ccrs.PlateCarree())
        clabels = plt.clabel(cs2, fmt='%d', colors='red',
        ...
        cs1 = ax.contour(lons, lats, avort.m*10**5, clevvort500, colors='grey',
                         linestyles='dashed', linewidths=1.5, transform=ccrs.PlateCarree())
        plt.clabel(cs1, fontsize=10, inline=1, inline_spacing=10,
    ...
    if level == 700 or level == 850 or level == 925:
        ...
        if te_option == True:
            # Calculate Theta-e
            rh = ds.Relative_humidity_isobaric.metpy.sel(
                vertical=level*100, lat=slice(85, 15), lon=slice(360-200, 360-10))
            td = mpcalc.dewpoint_from_relative_humidity(tmpk, rh)
            te = mpcalc.equivalent_potential_temperature(
                level*units.hPa, tmpk, td)
            ...
            cs2 = ax.contour(lons, lats, smooth_te.m, range(
                210, 360, tint), colors='orange', transform=ccrs.PlateCarree())
            clabels = plt.clabel(cs2, fmt='%d', colors='orange',
                                 inline_spacing=5, use_clabeltext=True, fontsize=22)
    ...
    if long_filenames == True:
        ...
        thumb_fname = '{0:%Y%m%d_%H}Z_'.format(
            date) + str(level) + 'mb_thumbnail.png'
    else:
        if hour == 12:
