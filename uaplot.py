...
def main():
    ...
    parser.add_option("--write-json", "--write-json", dest="write_json", action="store_true", help="Enable to output JSON file with references to the latest and archived image files.", default=False))
...
def get_filenames_json(path_root):
    ...
    files = [f for f in glob.glob('[0-9]{3}mb_([0-9]{12}z)\.png') if path.isfile(f)]
